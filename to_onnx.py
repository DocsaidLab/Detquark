...
class WarpMRZDetector(nn.Module):
    ...
    def forward(self, img: torch.Tensor):
        x = self.backbone(img)
        x = self.neck(x)
        pred_regions = self.head(x[0])
        pred_regions = pred_regions.softmax(1)
        return pred_regions[:, 2, :, :]


class WarpMRZScanner(nn.Module):
...
def main_torch2onnx(cfg_name: Union[str, Path]):
